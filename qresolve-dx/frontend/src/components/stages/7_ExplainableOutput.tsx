      ...
      <Card className={`p-0 border-t-4 overflow-hidden ${isQuantum ? 'border-quantum' : 'border-primary'}`}>
        <div className="px-8 py-6 bg-surface">
          ...
          <div className="space-y-8">
            <div>
              <h4 className="text-xs font-semibold text-green-700 uppercase tracking-wider mb-2">Supporting Evidence (Positive SHAP)</h4>
              <div className="divide-y divide-border border border-border bg-gray-50/50">
                {explainData.supporting_evidence.map((ev, i) => {
                  const width = `${Math.min(100, (ev.importance / maxImportance) * 100)}%`;
                  return (
                    <div key={i} className="py-3 px-4 flex flex-col gap-2">
                      <div className="flex justify-between items-center text-sm">
                        <span className="font-medium text-ink">{ev.label}</span>
                        <div className="flex items-center gap-3">
                          <span className="text-green-600 font-mono font-semibold">+{ev.importance.toFixed(3)}</span>
            ...
            <div>
              <h4 className="text-xs font-semibold text-red-700 uppercase tracking-wider mb-2">Counter Evidence (Negative SHAP)</h4>
              <div className="divide-y divide-border border border-border bg-gray-50/50">
                  ...
                  return (
                    <div key={i} className="py-3 px-4 flex flex-col gap-2">
                      <div className="flex justify-between items-center text-sm">
                        <span className="font-medium text-ink">{ev.label}</span>
                        <div className="flex items-center gap-3">
                          <span className="text-red-600 font-mono font-semibold">-{ev.importance.toFixed(3)}</span>
                          <span className="text-ink-muted bg-white border border-border px-2 py-0.5 text-xs font-mono">{ev.direction}</span>
                        </div>
                      </div>
                      <div className="w-full bg-gray-200 h-1.5 rounded-full overflow-hidden">
